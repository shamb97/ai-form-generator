...
async def create_study_from_conversation(
    request: dict,
    db = Depends(get_db)  # ← ADD DATABASE DEPENDENCY
):
    """
    ...
    try:
        ...
        print(f"📝 Creating study from conversation. Description length: {len(description)} chars")
        ...
        if forms_list and len(forms_list) > 0:
            ...
            print(f"💾 Saving to database: '{study_name}'")
            ...
            print(f"✅ Study created with ID: {study.id}")
            ...
            for idx, form_data in enumerate(forms_list):
                if isinstance(form_data, dict):
                    ...
                    print(f"   ✅ Form saved: {form_title} (ID: {db_form.id})")
            db.commit() 
            print(f"🎉 Complete study saved! Study ID: {study.id}, Forms: {len(forms_list)}")
            db.commit()
            ...
            print("⚠️ No forms in result, skipping database save")
...
async def submit_form_completion(
    ...
    try:
        ...
        print(f"✅ Form saved! Completion ID: {completion.id}")
...
async def execute_phase_transition(
    request: dict,
    db = Depends(get_db)  # ← Added proper dependency injection
):
    """
